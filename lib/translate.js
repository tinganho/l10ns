...
module.exports = function(grunt) {
  ...
  var options = _.defaults(grunt.config.get('translate').dist.options, {
    configDir: 'test' + '/' + translation,
    output: './translation/output',
    requireJS: true,
    translationFunctionName: 'gt',
    deleteLog: 'test' + '/' + translation + '/delete.log'
  });

  grunt.config.set('translate', 'update');
  grunt.config.set('translate', 'compile');
  grunt.config.set('translate', 'interface');

  grunt.registerMultiTask('translate', 'Your task description goes here.', function() {
    // Run appropiate sub task
    GruntTranslate[this.target](options);
  });
