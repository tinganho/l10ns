...
Init.prototype.init = function() {
  ...
  .then(function(programmingLanguage) {
    _this.programmingLanguages = programmingLanguage;
  })
  .fail(function(err) {
    console.log(err);
  });
...
Init.prototype._getDefaultProgrammingLanguage = function() {
  ...
  for(var i = 0; i < this.programmingLanguages.length; i++) {
    question += '  ' + this.programmingLanguages[i] + '\n';
    options += this.programmingLanguages[i] + ',';
  }
  options = options.slice(0, -1) + optionsEndWrap;
  ...
  (function ask() {
    ...
    _this.rl.question(question, function(programmingLanguage) {
      if(_this.programmingLanguages.indexOf(programmingLanguage) !== -1) {
        return deferred.resolve(programmingLanguage);
      }
      answeredWrong = true;
      ask();
    });
