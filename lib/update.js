...
Update.prototype.mergeUserInputs = function(newTranslations, oldTranslations, callback) {
  ...
  for(var key in deletedTranslations) {
    for(var file in deletedTranslations[key].files) {
      ...
      this.pushToUserInputStream(key, deletedTranslations[key].files[file]);
    }
  }

  this.executeUserInputStream(function(err, _newTranslations) {
...
Update.prototype.executeUserInputStream = function(newTranslations, oldTranslations, callback) {
  ...
  function recurse() {
    _this.getUserInputKey(this.deletedKeys.shift(), this.addedKeys.shift(), function(err, newKey, oldKey) {
      if(err) {
        hasError = true;
        return callback(err);
      }
...
Update.prototype.getUserInputKey = function(deletedKey, addedKeys, callback) {
  ...
  var question =
    ...
    + 'What dou you want to do with it?\n';
  ...
  rl.question(question, function(option) {
    callback(null, _addedKeys[option - 1], deletedKey);
    rl.close();
  });
