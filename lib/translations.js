...
Translations.prototype.getAllTranslations = function() {
  var files = glob.sync('./*.json', { cwd: cf.localesFolder });
  var locales = {};
  files.forEach(function(locale) {
    locales[path.basename(locale, '.json')] = require(path.join(cf.root, cf.localesFolder, locale));
  });
  return locales;
};
...
Translations.prototype.getTranslations = function(from, length, loc, raw) {
  ...
  if(!raw) {
    var keys = Compiler.prototype.formatTranslations(translations);
  ...
  if(length === 'all') {
    ...
    return translations.splice(from, length);
  }
